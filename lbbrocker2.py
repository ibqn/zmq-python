...
def worker_thread(worker_url, context, i):
    ...
    # set worker identity
    socket.identity = f"Worker-{i}".encode('ascii')
    ...
    try:
        while True:
            ...
            print(
                f"{socket.identity.decode()}: {request.decode()}"
            )
...
def client_thread(client_url, context, i):
    ...
    socket.identity = f"Client-{i}".encode('ascii')
    ...
    print(
        f"{socket.identity.decode()}: {reply.decode()}"
    )
...
def main():
    ...
    while True:
        ...
        if (backend in socks and socks[backend] == zmq.POLLIN):
            ...
            [worker_addr, empty, client_addr] = message[0:3]
